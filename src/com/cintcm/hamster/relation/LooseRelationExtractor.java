...
public class LooseRelationExtractor {
	...
	public LooseRelationExtractor(String txt, String docId) {
		this.text = txt;
	...
	private int extractRelations(int c1) {
		...
		while (c2 < words1.size()) {
			...
			if ((!obj.equalsIgnoreCase(s)) && (TCMNounDecider.isNoun(obj))) {
				boolean verbFound = false;
				for (String pred : words1) {
					if (VerbDecider.isVerb(pred) && !pred.equalsIgnoreCase(s)
						...
						if (TCMCoreDecider.isCore(s) && TCMCoreDecider.isCore(obj)){
							rel.setValue(3.0 );
						}else if ((TCMCoreDecider.isCore(s)) || (TCMCoreDecider.isCore(obj))) {
							rel.setValue(2.0 );
						}else{
				...
				if (!verbFound) {
					Relation rel = new Relation(s, words1.get(c2 - 1), obj)
							.setDocId(this.docId).setText(this.text);
					rel.setValue(0);
				}
			}
			c2++;
	...
	public static List<Relation> getRelations(File file, String docId) {
		...
		for (String sentence : sentences) {
			relations.addAll(new LooseRelationExtractor(sentence, docId)
					.getRelations());
		}
	...
	public static List<Relation> getRelationsFromExcel(File exl) {
		...
		try {
			...
			while (it.hasNext()) {
				...
				for (String sentence : sentences) {
					relations
							.addAll(new LooseRelationExtractor(sentence, docId)
									.getRelations());
				}
			}
	...
	public static void main(String[] args) {
		...
		LooseRelationExtractor extractor = new LooseRelationExtractor(txt, doc);
